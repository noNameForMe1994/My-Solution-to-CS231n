...
def adam(x, dx, config=None):
    ...
    config.setdefault('t', 0) # why should set t to 0? if use 1, next_x error is around 0.01
    ...
    # stored in config.                                                       #
    ###########################################################################
    m, v, t, b1, b2, lr = config['m'], config['v'], config['t'], config['beta1'], config['beta2'], config['learning_rate']
    m = b1 * m + (1 - b1) * dx
    v = b2 * v + (1 - b2) * dx ** 2
    t += 1
    m_u = m / (1 - b1 ** t)
